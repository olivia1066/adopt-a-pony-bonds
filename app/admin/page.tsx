...
// ── simple session-based admin gate ──
const ADMIN_KEY = 'pony_admin_auth'
    ...
    <main className="min-h-screen flex items-center justify-center font-sans"
      ...
      <div className="w-full max-w-sm px-8 py-12">
        <div className="text-center mb-10">
          <img src="/Logo.png" alt="Pony" style={{ height: '30px', margin: '0 auto' }} />
        </div>
        ...
        <div className="space-y-4">
          <div>
            ...
            <input
              ...
              style={{ backgroundColor: '#1E1B4B', border: '1px solid rgba(255,255,255,0.1)' }}
            />
          </div>
          {error && (
...
// ── types ──
type Campaign = {
  id: string; name: string; target_amount: number; raised_amount: number
  rate: number; duration: number; status: string; start_date: string
}
type Investor = {
  id: string; prenom: string; nom: string; email: string; telephone: string
  date_naissance: string; lieu_naissance: string; nationalite: string
  residence_fiscale: string; adresse: string; profession: string; revenus: string
  pep: string; document_type: string; document_numero: string; iban: string
  kyc_status: string; created_at: string
  investments?: { montant: number; campaigns?: { name: string } }[]
}
type Payment = {
  id: string; type: string; montant: number; date_prevue: string; statut: string
...
function AdminPanel() {
  ...
  const [newCampaign, setNewCampaign] = useState({ name: '', target: '', startDate: '' })
  ...
  async function createCampaign() {
    ...
    if (!error) {
      ...
      setNewCampaign({ name: '', target: '', startDate: '' })
      fetchAll()
    }
  }
            ...
            {showNewCampaign && (
              <div className="rounded-2xl p-6 mb-6"
                ...
                <div className="grid grid-cols-2 gap-4">
                  {[
                    { label: 'Campaign name', key: 'name', placeholder: 'Autumn 2026 Fleet' },
                    { label: 'Target amount (€)', key: 'target', placeholder: '500000' },
